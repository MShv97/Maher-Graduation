...
class SpecialtyService {
	...
	static async getById(user, _id) {
		const projection = Specialty.accessibleFieldsBy(user.abilities, 'view');
		const result = await Specialty.accessibleBy(user.abilities, 'view').findOne({ _id }, projection);
		if (!result) throw Exception.specialty.Not_Found;
		return { data: result };
	}
	...
	static async getByCriteria(user, criteria, pagination) {
		const conditions = (() => {
			const result = {
				...criteria,
				...Specialty.accessibleBy(user.abilities, 'view').getQuery(),
			};
		...
		const projection = Specialty.accessibleFieldsBy(user.abilities, 'view');
		const result = await Specialty.findAndCount({ conditions, projection, pagination });
